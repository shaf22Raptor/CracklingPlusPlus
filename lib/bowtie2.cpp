...
void bowtie2::run(unordered_map<string, unordered_map<string, string, std::less<>>, std::less<>>& candidateGuides)
{
	if (!toolIsSelected)
	{
	...
	while (pageEnd != candidateGuides.end())
	{
		...
		unordered_map<string, string, std::less<>> tempTargetDict_offset;
		// Open input file 
		std::ofstream inFile;
